...
impl<const N: usize, S: Simdness> Vector<N, i32, S>
where
    Usize<N>: VecLen,
{
            ...
            if S::IS_SIMD {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, i32, Simd>, Vector<N, i32, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i32; N], [i32; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i32, Simd>, Vector<N, i32, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i32; N], [i32; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i32, Simd>, Vector<N, i32, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i32, NonSimd>, Vector<N, i32, S>>(&Vector(array))
            }
        }
    }
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Self) -> Vector<N, u32, S> {
...
impl Scalar for i32 {
    type InnerSimdVec2 = [i32; 2];
    type InnerSimdVec3 = [i32; 3];
    type InnerSimdVec4 = [i32; 4];
