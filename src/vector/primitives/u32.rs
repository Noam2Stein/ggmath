            ...
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, u32, VecAligned>, Vector<N, u32, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[u32; N], [u32; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, u32, VecAligned>, Vector<N, u32, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[u32; N], [u32; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, u32, VecAligned>, Vector<N, u32, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, u32, VecPacked>, Vector<N, u32, A>>(&Vector(array))
            }
        }
    }
...
impl Scalar for u32 {
    type InnerAlignedVec2 = [u32; 2];
    type InnerAlignedVec3 = [u32; 3];
    type InnerAlignedVec4 = [u32; 4];
