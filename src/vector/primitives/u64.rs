    ...
    Usize<N>: VecLen,
{
    ...
    pub const MIN: Self = Self::const_from_array([u64::MIN; N]);
    /// A vector of all maximum values.
    pub const MAX: Self = Self::const_from_array([u64::MAX; N]);
...
impl Scalar for u64 {
    ...
    #[inline(always)]
    fn vec_from_array<const N: usize>(array: [u64; N]) -> Vector<N, u64, Simd>
    where
        Usize<N>: VecLen,
    {
        Vector(array)
    ...
    #[inline(always)]
    fn vec_as_array<const N: usize>(vec: Vector<N, u64, Simd>) -> [u64; N]
    where
        Usize<N>: VecLen,
    {
        vec.0
