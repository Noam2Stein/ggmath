    ...
    Usize<N>: VecLen,
{
    ...
    #[inline(always)]
    pub const fn const_from_array(array: [i64; N]) -> Self {
        unsafe {
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        let array = transmute_copy::<[i64; N], [i64; 2]>(&array);
                        let vec = Vector::<2, _, _>(array);

                        transmute_copy::<Vector<2, i64, VecAligned>, Vector<N, i64, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i64; N], [i64; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i64, VecAligned>, Vector<N, i64, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i64; N], [i64; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i64, VecAligned>, Vector<N, i64, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i64, VecPacked>, Vector<N, i64, A>>(&Vector(array))
            }
        }
    }

    #[inline(always)]
    pub const fn const_splat(value: i64) -> Self {
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Vector<N, i64, impl VecAlignment>) -> Vector<N, u64, A> {
    ...
    #[inline(always)]
    pub fn distance_sq(self, other: Vector<N, i64, impl VecAlignment>) -> u64 {
        self.abs_diff(other).mag_sq()
    }
}

impl Scalar for i64 {
    type InnerAlignedVec2 = [i64; 2];
    type InnerAlignedVec3 = [i64; 3];
    type InnerAlignedVec4 = [i64; 4];

    #[inline(always)]
    fn vec2_from_array(array: [i64; 2]) -> Vec2<i64> {
        Vector(array)
    }

    #[inline(always)]
    fn vec2_as_array(vec: Vec2<i64>) -> [i64; 2] {
        vec.0
    }

    #[inline(always)]
    fn vec3_from_array(array: [i64; 3]) -> Vec3<i64> {
        Vector(array)
    }

    #[inline(always)]
    fn vec3_as_array(vec: Vec3<i64>) -> [i64; 3] {
        vec.0
    }

    #[inline(always)]
    fn vec4_from_array(array: [i64; 4]) -> Vec4<i64> {
        Vector(array)
    }

    #[inline(always)]
    fn vec4_as_array(vec: Vec4<i64>) -> [i64; 4] {
        vec.0
    }
}
...
impl ScalarZero for i64 {
    ...
    const VEC2_ZERO: Vec2<i64> = Vec2::<i64>::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<i64> = Vec3::<i64>::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<i64> = Vec4::<i64>::const_from_array([0; 4]);
}

impl ScalarOne for i64 {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<i64> = Vec2::<i64>::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<i64> = Vec3::<i64>::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<i64> = Vec4::<i64>::const_from_array([1; 4]);

    const VEC2_X: Vec2<i64> = Vec2::<i64>::const_from_array([1, 0]);

    const VEC2_Y: Vec2<i64> = Vec2::<i64>::const_from_array([0, 1]);

    const VEC3_X: Vec3<i64> = Vec3::<i64>::const_from_array([1, 0, 0]);

    const VEC3_Y: Vec3<i64> = Vec3::<i64>::const_from_array([0, 1, 0]);

    const VEC3_Z: Vec3<i64> = Vec3::<i64>::const_from_array([0, 0, 1]);

    const VEC4_X: Vec4<i64> = Vec4::<i64>::const_from_array([1, 0, 0, 0]);

    const VEC4_Y: Vec4<i64> = Vec4::<i64>::const_from_array([0, 1, 0, 0]);

    const VEC4_Z: Vec4<i64> = Vec4::<i64>::const_from_array([0, 0, 1, 0]);

    const VEC4_W: Vec4<i64> = Vec4::<i64>::const_from_array([0, 0, 0, 1]);
}

impl ScalarNegOne for i64 {
    const NEG_ONE: Self = -1;

    const VEC2_NEG_ONE: Vec2<i64> = Vec2::<i64>::const_from_array([-1; 2]);
    const VEC3_NEG_ONE: Vec3<i64> = Vec3::<i64>::const_from_array([-1; 3]);
    const VEC4_NEG_ONE: Vec4<i64> = Vec4::<i64>::const_from_array([-1; 4]);

    const VEC2_NEG_X: Vec2<i64> = Vec2::<i64>::const_from_array([-1, 0]);

    const VEC2_NEG_Y: Vec2<i64> = Vec2::<i64>::const_from_array([0, -1]);

    const VEC3_NEG_X: Vec3<i64> = Vec3::<i64>::const_from_array([-1, 0, 0]);

    const VEC3_NEG_Y: Vec3<i64> = Vec3::<i64>::const_from_array([0, -1, 0]);

    const VEC3_NEG_Z: Vec3<i64> = Vec3::<i64>::const_from_array([0, 0, -1]);

    const VEC4_NEG_X: Vec4<i64> = Vec4::<i64>::const_from_array([-1, 0, 0, 0]);

    const VEC4_NEG_Y: Vec4<i64> = Vec4::<i64>::const_from_array([0, -1, 0, 0]);

    const VEC4_NEG_Z: Vec4<i64> = Vec4::<i64>::const_from_array([0, 0, -1, 0]);

    const VEC4_NEG_W: Vec4<i64> = Vec4::<i64>::const_from_array([0, 0, 0, -1]);
}
