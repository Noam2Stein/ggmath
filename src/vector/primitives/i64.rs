...
impl<const N: usize, S: Simdness> Vector<N, i64, S>
where
    Usize<N>: VecLen,
{
            ...
            if S::IS_SIMD {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, i64, Simd>, Vector<N, i64, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i64; N], [i64; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i64, Simd>, Vector<N, i64, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i64; N], [i64; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i64, Simd>, Vector<N, i64, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i64, NonSimd>, Vector<N, i64, S>>(&Vector(array))
            }
        }
    }
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Self) -> Vector<N, u64, S> {
...
impl Scalar for i64 {
    type InnerSimdVec2 = [i64; 2];
    type InnerSimdVec3 = [i64; 3];
    type InnerSimdVec4 = [i64; 4];
