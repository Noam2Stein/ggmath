            ...
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, i16, VecAligned>, Vector<N, i16, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i16; N], [i16; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i16, VecAligned>, Vector<N, i16, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i16; N], [i16; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i16, VecAligned>, Vector<N, i16, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i16, VecPacked>, Vector<N, i16, A>>(&Vector(array))
            }
        }
    }
...
impl Scalar for i16 {
    type InnerAlignedVec2 = [i16; 2];
    type InnerAlignedVec3 = [i16; 3];
    type InnerAlignedVec4 = [i16; 4];
