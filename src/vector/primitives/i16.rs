...
impl ScalarZero for i16 {
    ...
    const VEC2_ZERO: Vec2<i16> = Vec2::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<i16> = Vec3::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<i16> = Vec4::const_from_array([0; 4]);
}

impl ScalarOne for i16 {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<i16> = Vec2::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<i16> = Vec3::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<i16> = Vec4::const_from_array([1; 4]);

    const VEC2_x: Vec2<i16> = Vec2::const_from_array([1, 0]);

    const VEC2_y: Vec2<i16> = Vec2::const_from_array([0, 1]);

    const VEC3_x: Vec3<i16> = Vec3::const_from_array([1, 0, 0]);

    const VEC3_y: Vec3<i16> = Vec3::const_from_array([0, 1, 0]);

    const VEC3_z: Vec3<i16> = Vec3::const_from_array([0, 0, 1]);

    const VEC4_x: Vec4<i16> = Vec4::const_from_array([1, 0, 0, 0]);

    const VEC4_y: Vec4<i16> = Vec4::const_from_array([0, 1, 0, 0]);

    const VEC4_z: Vec4<i16> = Vec4::const_from_array([0, 0, 1, 0]);

    const VEC4_w: Vec4<i16> = Vec4::const_from_array([0, 0, 0, 1]);
}

impl ScalarNegOne for i16 {
    const NEG_ONE: Self = -1;

    const VEC2_NEG_ONE: Vec2<i16> = Vec2::const_from_array([-1; 2]);
    const VEC3_NEG_ONE: Vec3<i16> = Vec3::const_from_array([-1; 3]);
    const VEC4_NEG_ONE: Vec4<i16> = Vec4::const_from_array([-1; 4]);

    const VEC2_NEG_x: Vec2<i16> = Vec2::const_from_array([-1, 0]);

    const VEC2_NEG_y: Vec2<i16> = Vec2::const_from_array([0, -1]);

    const VEC3_NEG_x: Vec3<i16> = Vec3::const_from_array([-1, 0, 0]);

    const VEC3_NEG_y: Vec3<i16> = Vec3::const_from_array([0, -1, 0]);

    const VEC3_NEG_z: Vec3<i16> = Vec3::const_from_array([0, 0, -1]);

    const VEC4_NEG_x: Vec4<i16> = Vec4::const_from_array([-1, 0, 0, 0]);

    const VEC4_NEG_y: Vec4<i16> = Vec4::const_from_array([0, -1, 0, 0]);

    const VEC4_NEG_z: Vec4<i16> = Vec4::const_from_array([0, 0, -1, 0]);

    const VEC4_NEG_w: Vec4<i16> = Vec4::const_from_array([0, 0, 0, -1]);
}
