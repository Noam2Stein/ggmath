...
impl<const N: usize, S: Simdness> Vector<N, i16, S>
where
    Usize<N>: VecLen,
{
    ...
    #[inline(always)]
    pub const fn const_from_array(array: [i16; N]) -> Self {
        unsafe {
            if S::IS_SIMD {
                match N {
                    2 => {
                        let array = transmute_copy::<[i16; N], [i16; 2]>(&array);
                        let vec = Vector::<2, _, _>(array);

                        transmute_copy::<Vector<2, i16, Simd>, Vector<N, i16, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i16; N], [i16; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i16, Simd>, Vector<N, i16, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i16; N], [i16; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i16, Simd>, Vector<N, i16, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i16, NonSimd>, Vector<N, i16, S>>(&Vector(array))
            }
        }
    }
    ...
    /// When rust stabilizes const traits, this will be deleted.
    #[inline(always)]
    pub const fn const_splat(value: i16) -> Self {
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Self) -> Vector<N, u16, S> {
...
impl Scalar for i16 {
    type InnerSimdVec2 = [i16; 2];
    type InnerSimdVec3 = [i16; 3];
    type InnerSimdVec4 = [i16; 4];

    #[inline(always)]
    fn vec2_from_array(array: [i16; 2]) -> Vec2<i16> {
        Vector(array)
    }

    #[inline(always)]
    fn vec2_as_array(vec: Vec2<i16>) -> [i16; 2] {
        vec.0
    }

    #[inline(always)]
    fn vec3_from_array(array: [i16; 3]) -> Vec3<i16> {
        Vector(array)
    }

    #[inline(always)]
    fn vec3_as_array(vec: Vec3<i16>) -> [i16; 3] {
        vec.0
    }

    #[inline(always)]
    fn vec4_from_array(array: [i16; 4]) -> Vec4<i16> {
        Vector(array)
    }

    #[inline(always)]
    fn vec4_as_array(vec: Vec4<i16>) -> [i16; 4] {
        vec.0
    }
}
...
impl ScalarZero for i16 {
    ...
    const VEC2_ZERO: Vec2<i16> = Vec2::<i16>::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<i16> = Vec3::<i16>::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<i16> = Vec4::<i16>::const_from_array([0; 4]);
}

impl ScalarOne for i16 {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<i16> = Vec2::<i16>::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<i16> = Vec3::<i16>::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<i16> = Vec4::<i16>::const_from_array([1; 4]);

    const VEC2_X: Vec2<i16> = Vec2::<i16>::const_from_array([1, 0]);

    const VEC2_Y: Vec2<i16> = Vec2::<i16>::const_from_array([0, 1]);

    const VEC3_X: Vec3<i16> = Vec3::<i16>::const_from_array([1, 0, 0]);

    const VEC3_Y: Vec3<i16> = Vec3::<i16>::const_from_array([0, 1, 0]);

    const VEC3_Z: Vec3<i16> = Vec3::<i16>::const_from_array([0, 0, 1]);

    const VEC4_X: Vec4<i16> = Vec4::<i16>::const_from_array([1, 0, 0, 0]);

    const VEC4_Y: Vec4<i16> = Vec4::<i16>::const_from_array([0, 1, 0, 0]);

    const VEC4_Z: Vec4<i16> = Vec4::<i16>::const_from_array([0, 0, 1, 0]);

    const VEC4_W: Vec4<i16> = Vec4::<i16>::const_from_array([0, 0, 0, 1]);
}

impl ScalarNegOne for i16 {
    const NEG_ONE: Self = -1;

    const VEC2_NEG_ONE: Vec2<i16> = Vec2::<i16>::const_from_array([-1; 2]);
    const VEC3_NEG_ONE: Vec3<i16> = Vec3::<i16>::const_from_array([-1; 3]);
    const VEC4_NEG_ONE: Vec4<i16> = Vec4::<i16>::const_from_array([-1; 4]);

    const VEC2_NEG_X: Vec2<i16> = Vec2::<i16>::const_from_array([-1, 0]);

    const VEC2_NEG_Y: Vec2<i16> = Vec2::<i16>::const_from_array([0, -1]);

    const VEC3_NEG_X: Vec3<i16> = Vec3::<i16>::const_from_array([-1, 0, 0]);

    const VEC3_NEG_Y: Vec3<i16> = Vec3::<i16>::const_from_array([0, -1, 0]);

    const VEC3_NEG_Z: Vec3<i16> = Vec3::<i16>::const_from_array([0, 0, -1]);

    const VEC4_NEG_X: Vec4<i16> = Vec4::<i16>::const_from_array([-1, 0, 0, 0]);

    const VEC4_NEG_Y: Vec4<i16> = Vec4::<i16>::const_from_array([0, -1, 0, 0]);

    const VEC4_NEG_Z: Vec4<i16> = Vec4::<i16>::const_from_array([0, 0, -1, 0]);

    const VEC4_NEG_W: Vec4<i16> = Vec4::<i16>::const_from_array([0, 0, 0, -1]);
}
