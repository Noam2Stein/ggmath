    ...
    Usize<N>: VecLen,
{
    ...
    pub const fn const_from_array(array: [i8; N]) -> Self {
        unsafe {
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        let array = transmute_copy::<[i8; N], [i8; 2]>(&array);
                        let vec = Vector::<2, _, _>(array);

                        transmute_copy::<Vector<2, i8, VecAligned>, Vector<N, i8, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i8; N], [i8; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i8, VecAligned>, Vector<N, i8, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i8; N], [i8; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i8, VecAligned>, Vector<N, i8, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i8, VecPacked>, Vector<N, i8, A>>(&Vector(array))
            }
        }
    }

    #[inline(always)]
    pub const fn const_splat(value: i8) -> Self {
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Vector<N, i8, impl VecAlignment>) -> Vector<N, u8, A> {
    ...
    #[inline(always)]
    pub fn distance_sq(self, other: Vector<N, i8, impl VecAlignment>) -> u8 {
        self.abs_diff(other).mag_sq()
    }
}

impl Scalar for i8 {
    type InnerAlignedVec2 = [i8; 2];
    type InnerAlignedVec3 = [i8; 3];
    type InnerAlignedVec4 = [i8; 4];

    #[inline(always)]
    fn vec2_from_array(array: [i8; 2]) -> Vec2<i8> {
        Vector(array)
    }

    #[inline(always)]
    fn vec2_as_array(vec: Vec2<i8>) -> [i8; 2] {
        vec.0
    }

    #[inline(always)]
    fn vec3_from_array(array: [i8; 3]) -> Vec3<i8> {
        Vector(array)
    }

    #[inline(always)]
    fn vec3_as_array(vec: Vec3<i8>) -> [i8; 3] {
        vec.0
    }

    #[inline(always)]
    fn vec4_from_array(array: [i8; 4]) -> Vec4<i8> {
        Vector(array)
    }

    #[inline(always)]
    fn vec4_as_array(vec: Vec4<i8>) -> [i8; 4] {
        vec.0
    }
}
...
impl ScalarZero for i8 {
    ...
    const VEC2_ZERO: Vec2<i8> = Vec2::<i8>::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<i8> = Vec3::<i8>::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<i8> = Vec4::<i8>::const_from_array([0; 4]);
}

impl ScalarOne for i8 {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<i8> = Vec2::<i8>::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<i8> = Vec3::<i8>::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<i8> = Vec4::<i8>::const_from_array([1; 4]);

    const VEC2_X: Vec2<i8> = Vec2::<i8>::const_from_array([1, 0]);

    const VEC2_Y: Vec2<i8> = Vec2::<i8>::const_from_array([0, 1]);

    const VEC3_X: Vec3<i8> = Vec3::<i8>::const_from_array([1, 0, 0]);

    const VEC3_Y: Vec3<i8> = Vec3::<i8>::const_from_array([0, 1, 0]);

    const VEC3_Z: Vec3<i8> = Vec3::<i8>::const_from_array([0, 0, 1]);

    const VEC4_X: Vec4<i8> = Vec4::<i8>::const_from_array([1, 0, 0, 0]);

    const VEC4_Y: Vec4<i8> = Vec4::<i8>::const_from_array([0, 1, 0, 0]);

    const VEC4_Z: Vec4<i8> = Vec4::<i8>::const_from_array([0, 0, 1, 0]);

    const VEC4_W: Vec4<i8> = Vec4::<i8>::const_from_array([0, 0, 0, 1]);
}

impl ScalarNegOne for i8 {
    const NEG_ONE: Self = -1;

    const VEC2_NEG_ONE: Vec2<i8> = Vec2::<i8>::const_from_array([-1; 2]);
    const VEC3_NEG_ONE: Vec3<i8> = Vec3::<i8>::const_from_array([-1; 3]);
    const VEC4_NEG_ONE: Vec4<i8> = Vec4::<i8>::const_from_array([-1; 4]);

    const VEC2_NEG_X: Vec2<i8> = Vec2::<i8>::const_from_array([-1, 0]);

    const VEC2_NEG_Y: Vec2<i8> = Vec2::<i8>::const_from_array([0, -1]);

    const VEC3_NEG_X: Vec3<i8> = Vec3::<i8>::const_from_array([-1, 0, 0]);

    const VEC3_NEG_Y: Vec3<i8> = Vec3::<i8>::const_from_array([0, -1, 0]);

    const VEC3_NEG_Z: Vec3<i8> = Vec3::<i8>::const_from_array([0, 0, -1]);

    const VEC4_NEG_X: Vec4<i8> = Vec4::<i8>::const_from_array([-1, 0, 0, 0]);

    const VEC4_NEG_Y: Vec4<i8> = Vec4::<i8>::const_from_array([0, -1, 0, 0]);

    const VEC4_NEG_Z: Vec4<i8> = Vec4::<i8>::const_from_array([0, 0, -1, 0]);

    const VEC4_NEG_W: Vec4<i8> = Vec4::<i8>::const_from_array([0, 0, 0, -1]);
}
