...
impl<const N: usize, S: Simdness> Vector<N, i128, S>
where
    Usize<N>: VecLen,
{
            ...
            if S::IS_SIMD {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, i128, Simd>, Vector<N, i128, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[i128; N], [i128; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, i128, Simd>, Vector<N, i128, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[i128; N], [i128; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, i128, Simd>, Vector<N, i128, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, i128, NonSimd>, Vector<N, i128, S>>(&Vector(array))
            }
        }
    }
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Self) -> Vector<N, u128, S> {
...
impl Scalar for i128 {
    ...
    #[inline(always)]
    fn vec_from_array<const N: usize>(array: [i128; N]) -> Vector<N, i128, Simd>
    where
        Usize<N>: VecLen,
    {
        Vector(array)
    ...
    #[inline(always)]
    fn vec_as_array<const N: usize>(vec: Vector<N, i128, Simd>) -> [i128; N]
    where
        Usize<N>: VecLen,
    {
        vec.0
