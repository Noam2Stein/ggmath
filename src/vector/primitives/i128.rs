...
impl Scalar for i128 {
    // AVX2 doesn't provide i128x2 instructions but the alignment is worth it for bit-ops
    cfg_if! {
        if #[cfg(any(
    ...
    // AVX512 doesn't provide i128x4 instructions but the alignment is worth it for bit-ops
    cfg_if! {
        if #[cfg(any(
