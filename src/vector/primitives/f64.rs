            ...
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, f64, VecAligned>, Vector<N, f64, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[f64; N], [f64; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, f64, VecAligned>, Vector<N, f64, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[f64; N], [f64; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, f64, VecAligned>, Vector<N, f64, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, f64, VecPacked>, Vector<N, f64, A>>(&Vector(array))
            }
        }
    }
...
impl Scalar for f64 {
    type InnerAlignedVec2 = [f64; 2];
    type InnerAlignedVec3 = [f64; 3];
    type InnerAlignedVec4 = [f64; 4];
