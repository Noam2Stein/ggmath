...
impl<const N: usize> FloatElementOfVector<N, NonSimd> for f64 {
    #[inline(always)]
    fn vec_floor(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::floor)
    }

    #[inline(always)]
    fn vec_ceil(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::ceil)
    }

    #[inline(always)]
    fn vec_round(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::round)
    }

    #[inline(always)]
    fn vec_trunc(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::trunc)
    }

    #[inline(always)]
    fn vec_fract(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::fract)
    }

    #[inline(always)]
    fn vec_mul_add(
        vec: Vector<N, f64, NonSimd>,
        a: Vector<N, f64, NonSimd>,
        b: Vector<N, f64, NonSimd>,
    ) -> Vector<N, f64, NonSimd> {
    ...
    #[inline(always)]
    fn vec_sqrt(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::sqrt)
    }

    #[inline(always)]
    fn vec_sin(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::sin)
    }

    #[inline(always)]
    fn vec_cos(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::cos)
    }

    #[inline(always)]
    fn vec_tan(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::tan)
    }

    #[inline(always)]
    fn vec_asin(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::asin)
    }

    #[inline(always)]
    fn vec_acos(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::acos)
    }

    #[inline(always)]
    fn vec_atan(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::atan)
    }

    #[inline(always)]
    fn vec_recip(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::recip)
    }
    ...
    fn vec_clamp(
        vec: Vector<N, f64, NonSimd>,
        min: Vector<N, f64, NonSimd>,
        max: Vector<N, f64, NonSimd>,
    ) -> Vector<N, f64, NonSimd> {
    ...
    #[inline(always)]
    fn vec_abs(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::abs)
    }

    #[inline(always)]
    fn vec_signum(vec: Vector<N, f64, NonSimd>) -> Vector<N, f64, NonSimd> {
        vec.map(f64::signum)
    }
    ...
    fn vec_copysign(
        vec: Vector<N, f64, NonSimd>,
        sign: Vector<N, f64, NonSimd>,
    ) -> Vector<N, f64, NonSimd> {
        vec.zip(sign).map(|(x, sign)| f64::copysign(x, sign))
    }

    #[inline(always)]
    fn vec_sum(vec: Vector<N, f64, NonSimd>) -> f64 {
        vec.iter().sum()
    }

    #[inline(always)]
    fn vec_product(vec: Vector<N, f64, NonSimd>) -> f64 {
        vec.iter().product()
    }
}
