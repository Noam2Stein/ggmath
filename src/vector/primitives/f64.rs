    ...
    Usize<N>: VecLen,
{
    ...
    pub const MIN: Self = Self::const_from_array([f64::MIN; N]);
    /// A vector of all maximum values.
    pub const MAX: Self = Self::const_from_array([f64::MAX; N]);
    /// A vector with all elements set to `NaN`.
    pub const NAN: Self = Self::const_from_array([f64::NAN; N]);
    /// A vector with all elements set to `Infinity`.
    pub const INFINITY: Self = Self::const_from_array([f64::INFINITY; N]);
    /// A vector with all elements set to `-Infinity`.
    pub const NEG_INFINITY: Self = Self::const_from_array([f64::NEG_INFINITY; N]);
    /// A vector with all elements set to `Epsilon`.
    pub const EPSILON: Self = Self::const_from_array([f64::EPSILON; N]);
...
impl Scalar for f64 {
    ...
    #[inline(always)]
    fn vec_from_array<const N: usize>(array: [f64; N]) -> Vector<N, f64, Simd>
    where
        Usize<N>: VecLen,
    {
        Vector(array)
    ...
    #[inline(always)]
    fn vec_as_array<const N: usize>(vec: Vector<N, f64, Simd>) -> [f64; N]
    where
        Usize<N>: VecLen,
    {
        vec.0
