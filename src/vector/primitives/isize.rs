    ...
    Usize<N>: VecLen,
{
    ...
    #[inline(always)]
    pub const fn const_from_array(array: [isize; N]) -> Self {
        unsafe {
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        let array = transmute_copy::<[isize; N], [isize; 2]>(&array);
                        let vec = Vector::<2, _, _>(array);

                        transmute_copy::<Vector<2, isize, VecAligned>, Vector<N, isize, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[isize; N], [isize; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, isize, VecAligned>, Vector<N, isize, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[isize; N], [isize; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, isize, VecAligned>, Vector<N, isize, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, isize, VecPacked>, Vector<N, isize, A>>(&Vector(array))
            }
        }
    }

    #[inline(always)]
    pub const fn const_splat(value: isize) -> Self {
    ...
    #[inline(always)]
    pub fn abs_diff(self, other: Vector<N, isize, impl VecAlignment>) -> Vector<N, usize, A> {
    ...
    #[inline(always)]
    pub fn distance_sq(self, other: Vector<N, isize, impl VecAlignment>) -> usize {
        self.abs_diff(other).mag_sq()
    }
}

impl Scalar for isize {
    type InnerAlignedVec2 = [isize; 2];
    type InnerAlignedVec3 = [isize; 3];
    type InnerAlignedVec4 = [isize; 4];
...
impl ScalarZero for isize {
    ...
    const VEC2_ZERO: Vec2<isize> = Vec2::<isize>::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<isize> = Vec3::<isize>::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<isize> = Vec4::<isize>::const_from_array([0; 4]);
}

impl ScalarOne for isize {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<isize> = Vec2::<isize>::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<isize> = Vec3::<isize>::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<isize> = Vec4::<isize>::const_from_array([1; 4]);

    const VEC2_X: Vec2<isize> = Vec2::<isize>::const_from_array([1, 0]);

    const VEC2_Y: Vec2<isize> = Vec2::<isize>::const_from_array([0, 1]);

    const VEC3_X: Vec3<isize> = Vec3::<isize>::const_from_array([1, 0, 0]);

    const VEC3_Y: Vec3<isize> = Vec3::<isize>::const_from_array([0, 1, 0]);

    const VEC3_Z: Vec3<isize> = Vec3::<isize>::const_from_array([0, 0, 1]);

    const VEC4_X: Vec4<isize> = Vec4::<isize>::const_from_array([1, 0, 0, 0]);

    const VEC4_Y: Vec4<isize> = Vec4::<isize>::const_from_array([0, 1, 0, 0]);

    const VEC4_Z: Vec4<isize> = Vec4::<isize>::const_from_array([0, 0, 1, 0]);

    const VEC4_W: Vec4<isize> = Vec4::<isize>::const_from_array([0, 0, 0, 1]);
}

impl ScalarNegOne for isize {
    const NEG_ONE: Self = -1;

    const VEC2_NEG_ONE: Vec2<isize> = Vec2::<isize>::const_from_array([-1; 2]);
    const VEC3_NEG_ONE: Vec3<isize> = Vec3::<isize>::const_from_array([-1; 3]);
    const VEC4_NEG_ONE: Vec4<isize> = Vec4::<isize>::const_from_array([-1; 4]);

    const VEC2_NEG_X: Vec2<isize> = Vec2::<isize>::const_from_array([-1, 0]);

    const VEC2_NEG_Y: Vec2<isize> = Vec2::<isize>::const_from_array([0, -1]);

    const VEC3_NEG_X: Vec3<isize> = Vec3::<isize>::const_from_array([-1, 0, 0]);

    const VEC3_NEG_Y: Vec3<isize> = Vec3::<isize>::const_from_array([0, -1, 0]);

    const VEC3_NEG_Z: Vec3<isize> = Vec3::<isize>::const_from_array([0, 0, -1]);

    const VEC4_NEG_X: Vec4<isize> = Vec4::<isize>::const_from_array([-1, 0, 0, 0]);

    const VEC4_NEG_Y: Vec4<isize> = Vec4::<isize>::const_from_array([0, -1, 0, 0]);

    const VEC4_NEG_Z: Vec4<isize> = Vec4::<isize>::const_from_array([0, 0, -1, 0]);

    const VEC4_NEG_W: Vec4<isize> = Vec4::<isize>::const_from_array([0, 0, 0, -1]);
}
