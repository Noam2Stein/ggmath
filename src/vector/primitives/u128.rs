            ...
            if A::IS_ALIGNED {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, u128, VecAligned>, Vector<N, u128, A>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[u128; N], [u128; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, u128, VecAligned>, Vector<N, u128, A>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[u128; N], [u128; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, u128, VecAligned>, Vector<N, u128, A>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, u128, VecPacked>, Vector<N, u128, A>>(&Vector(array))
            }
        }
    }
...
impl Scalar for u128 {
    type InnerAlignedVec2 = [u128; 2];
    type InnerAlignedVec3 = [u128; 3];
    type InnerAlignedVec4 = [u128; 4];
