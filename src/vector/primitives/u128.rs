...
impl<const N: usize, S: Simdness> Vector<N, u128, S>
where
    Usize<N>: VecLen,
{
    ...
    #[inline(always)]
    pub const fn const_from_array(array: [u128; N]) -> Self {
        unsafe {
            if S::IS_SIMD {
                match N {
                    2 => {
                        let array = transmute_copy::<[u128; N], [u128; 2]>(&array);
                        let vec = Vector::<2, _, _>(array);

                        transmute_copy::<Vector<2, u128, Simd>, Vector<N, u128, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[u128; N], [u128; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, u128, Simd>, Vector<N, u128, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[u128; N], [u128; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, u128, Simd>, Vector<N, u128, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, u128, NonSimd>, Vector<N, u128, S>>(&Vector(array))
            }
        }
    }
    ...
    /// When rust stabilizes const traits, this will be deleted.
    #[inline(always)]
    pub const fn const_splat(value: u128) -> Self {
...
impl Scalar for u128 {
    type InnerSimdVec2 = [u128; 2];
    type InnerSimdVec3 = [u128; 3];
    type InnerSimdVec4 = [u128; 4];

    #[inline(always)]
    fn vec2_from_array(array: [u128; 2]) -> Vec2<u128> {
        Vector(array)
    }

    #[inline(always)]
    fn vec2_as_array(vec: Vec2<u128>) -> [u128; 2] {
        vec.0
    }

    #[inline(always)]
    fn vec3_from_array(array: [u128; 3]) -> Vec3<u128> {
        Vector(array)
    }

    #[inline(always)]
    fn vec3_as_array(vec: Vec3<u128>) -> [u128; 3] {
        vec.0
    }

    #[inline(always)]
    fn vec4_from_array(array: [u128; 4]) -> Vec4<u128> {
        Vector(array)
    }

    #[inline(always)]
    fn vec4_as_array(vec: Vec4<u128>) -> [u128; 4] {
        vec.0
    }
}
...
impl ScalarZero for u128 {
    ...
    const VEC2_ZERO: Vec2<u128> = Vec2::<u128>::const_from_array([0; 2]);
    const VEC3_ZERO: Vec3<u128> = Vec3::<u128>::const_from_array([0; 3]);
    const VEC4_ZERO: Vec4<u128> = Vec4::<u128>::const_from_array([0; 4]);
}

impl ScalarOne for u128 {
    const ONE: Self = 1;

    const VEC2_ONE: Vec2<u128> = Vec2::<u128>::const_from_array([1; 2]);
    const VEC3_ONE: Vec3<u128> = Vec3::<u128>::const_from_array([1; 3]);
    const VEC4_ONE: Vec4<u128> = Vec4::<u128>::const_from_array([1; 4]);

    const VEC2_X: Vec2<u128> = Vec2::<u128>::const_from_array([1, 0]);

    const VEC2_Y: Vec2<u128> = Vec2::<u128>::const_from_array([0, 1]);

    const VEC3_X: Vec3<u128> = Vec3::<u128>::const_from_array([1, 0, 0]);

    const VEC3_Y: Vec3<u128> = Vec3::<u128>::const_from_array([0, 1, 0]);

    const VEC3_Z: Vec3<u128> = Vec3::<u128>::const_from_array([0, 0, 1]);

    const VEC4_X: Vec4<u128> = Vec4::<u128>::const_from_array([1, 0, 0, 0]);

    const VEC4_Y: Vec4<u128> = Vec4::<u128>::const_from_array([0, 1, 0, 0]);

    const VEC4_Z: Vec4<u128> = Vec4::<u128>::const_from_array([0, 0, 1, 0]);

    const VEC4_W: Vec4<u128> = Vec4::<u128>::const_from_array([0, 0, 0, 1]);
}
