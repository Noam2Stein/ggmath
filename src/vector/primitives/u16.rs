            ...
            if S::IS_SIMD {
                match N {
                    2 => {
                        ...
                        transmute_copy::<Vector<2, u16, Simd>, Vector<N, u16, S>>(&vec)
                    }
                    3 => {
                        let array = transmute_copy::<[u16; N], [u16; 3]>(&array);
                        let vec = Vector::<3, _, _>(array);

                        transmute_copy::<Vector<3, u16, Simd>, Vector<N, u16, S>>(&vec)
                    }
                    4 => {
                        let array = transmute_copy::<[u16; N], [u16; 4]>(&array);
                        let vec = Vector::<4, _, _>(array);

                        transmute_copy::<Vector<4, u16, Simd>, Vector<N, u16, S>>(&vec)
                    }
                    _ => panic!("unusual vector type"),
                }
            } else {
                transmute_copy::<Vector<N, u16, NonSimd>, Vector<N, u16, S>>(&Vector(array))
            }
        }
    }
...
impl Scalar for u16 {
    type InnerSimdVec2 = [u16; 2];
    type InnerSimdVec3 = [u16; 3];
    type InnerSimdVec4 = [u16; 4];
