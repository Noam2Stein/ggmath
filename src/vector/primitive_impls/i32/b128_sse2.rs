...
unsafe impl SoundVectorRepr<4, i32> for __m128i {}

// SAFETY: __m128i can contain exactly 4 i32s, so it does begin with 3 i32s
unsafe impl SoundVectorRepr<3, i32> for __m128i {}
...
impl SimdBehaviour<4> for i32 {
    ...
    fn vec_from_array(array: [Self; 4]) -> Vec4<Self> {
        Vector::from_repr(unsafe { _mm_set_epi32(array[3], array[2], array[1], array[0]) })
    }

    #[inline(always)]
    fn vec_splat(value: Self) -> Vec4<Self> {
        Vector::from_repr(unsafe { _mm_set1_epi32(value) })
    }
    ...
    fn vec_neg(vec: Vec4<Self>) -> Vec4<Self> {
        Vector::from_repr(unsafe { _mm_sub_epi32(vec4!(0).repr(), vec.repr()) })
    }
...
impl SimdBehaviour<3> for i32 {
    ...
    fn vec_from_array(array: [Self; 3]) -> Vec3<Self> {
        Vector::from_repr(unsafe { _mm_set_epi32(array[2], array[2], array[1], array[0]) })
    }
    ...
    fn vec_neg(vec: Vec3<Self>) -> Vec3<Self> {
        Vector::from_repr(unsafe { _mm_sub_epi32(vec3!(0).repr(), vec.repr()) })
    }
