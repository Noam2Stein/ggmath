    ...
    Usize<N>: VecLen,
{
	...
	/// Returns `self == other` and supports const contexts.
	#[inline(always)]
	pub const fn const_eq(self, other: Vector<N, i16, impl VecAlignment>) -> bool {
	    let mut i = 0;
	    while i < N {
	...
	/// Returns `self != other` and supports const contexts.
	#[inline(always)]
	pub const fn const_ne(self, other: Vector<N, i16, impl VecAlignment>) -> bool {
	    let mut i = 0;
	    while i < N {
	        if self.as_array()[i] != other.as_array()[i] {
	            return true;
	...
	pub const fn const_eq_mask(
	    self,
	    other: Vector<N, i16, impl VecAlignment>,
	) -> Vector<N, bool, A> {
