...
/// A trait for a `Right` constant where right is the negative direction.
#[cfg(feature = "left")]
pub trait NegativeRight: Construct {
    /// A value that points right with magnitude `1` where right is the negative direction.
    const RIGHT: Self;
}

/// A trait for a `Left` constant where left is the negative direction.
#[cfg(feature = "right")]
pub trait NegativeLeft: Construct {
    /// A value that points left with magnitude `1` where left is the negative direction.
    const LEFT: Self;
}
...
/// A trait for a `Up` constant where up is the negative direction.
#[cfg(feature = "down")]
pub trait NegativeUp: Construct {
    /// A value that points up with magnitude `1` where up is the negative direction.
    const UP: Self;
}

/// A trait for a `Down` constant where down is the negative direction.
#[cfg(feature = "up")]
pub trait NegativeDown: Construct {
    /// A value that points down with magnitude `1` where down is the negative direction.
    const DOWN: Self;
}
...
/// A trait for a `Forward` constant where forwards is the negative direction.
#[cfg(feature = "backwards")]
pub trait NegativeForward: Construct {
    /// A value that points forwards with magnitude `1` where forwards is the negative direction.
    const FORWARD: Self;
}

/// A trait for a `Backward` constant where backwards is the negative direction.
#[cfg(feature = "forwards")]
pub trait NegativeBackward: Construct {
    /// A value that points backwards with magnitude `1` where backwards is the negative direction.
    const BACKWARD: Self;
}
...
impl<T: ScalarZero + ScalarOne, A: VecAlignment> PositiveForward for Vector<2, T, A> {
    const FORWARD: Self = vec2g!(T::ZERO, T::ZERO);
}

#[cfg(feature = "backwards")]
impl<T: ScalarZero + ScalarOne, A: VecAlignment> PositiveBackward for Vector<2, T, A> {
    const BACKWARD: Self = vec2g!(T::ZERO, T::ZERO);
}
