...
use crate::{
    Alignment, Dim, Element, Matrix, SupportedLength, Vector,
...
#[expect(private_bounds)]
impl<Wide, A: Alignment> Matrix<4, Wide, A>
where
    Wide: WideTy,
{
    #[inline(always)]
...
macro_rules! impl_select {
    ($Mask:ident) => {
        impl<const N: usize, Wide, A: Alignment> Select<Matrix<N, Wide, A>> for wide::$Mask
        where
            Dim<N>: SupportedLength,
            ...
            Wide: WideTy,
        {
            ...
            fn select(
                self,
                if_true: Matrix<N, Wide, A>,
                if_false: Matrix<N, Wide, A>,
            ) -> Matrix<N, Wide, A> {
                specialize!(Matrix::<N, Wide, A>::scalar_select_backend::<wide::$Mask>(
    ...
    fn test_scalar_select() {
        for_types!(|N| {
            for (mask, [if_true, if_false]) in
                random_iter::<(i32x4, [Matrix<N, f32x4, Unaligned>; 2])>()
            {
                let mask = mask.is_negative();

                assert_test_eq!(
                    mask.select(if_true, if_false),
                    Matrix::from_lane_fn(|lane| if mask.as_array()[lane].is_negative() {
                        if_true.lane(lane)
                    } else {
                        if_false.lane(lane)
                    })
