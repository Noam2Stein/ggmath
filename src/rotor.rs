...
mod tests {
    ...
    use crate::{
        Aligned, Mask, Matrix, Rotor, Rotor2, Rotor2A, Rotor3, Rotor3A, Unaligned, Vec2A, Vec4A,
        Vector,
    ...
    fn test_debug() {
        for_types!(|T: PrimitiveNumber, A| {
            ...
            assert_eq!(
                format!("{rotor:?}"),
                format!("Rotor {{ xy: {:?}, s: {:?} }}", rotor.xy, rotor.s)
            );
            ...
            assert_eq!(
                ...
                format!(
                    "Rotor {{ xy: {:?}, xz: {:?}, yz: {:?}, s: {:?} }}",
                    rotor.xy, rotor.xz, rotor.yz, rotor.s
                )
            );
        });
    }
    ...
    fn test_vector_mul() {
        for_types!(|N: TwoOrThree, T: PrimitiveFloat, A| {
            ...
            for (vector, rotor) in random_iter::<(Vector<N, T, A>, Rotor<N, T, A>)>() {
                assert_test_eq!(vector * -rotor, vector * rotor);
            }
            for (vector, rotor) in random_iter::<(Vector<N, T, A>, Rotor<N, T, A>)>() {
