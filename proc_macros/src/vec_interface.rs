...
fn impl_block(input: &VecInterface) -> TokenStream {
    ...
    let interface_generics_args = generic_args(&input.generics);

    let output_impl_generics = quote_spanned! {
        input.ident.span() =>
        <
            const N: usize,
            T: #scalar_trait_ident<#(#interface_generics_args), *>,
            A: VecAlignment
    ...
    let output_impl_types = if input.impl_trait.is_some() {
        ...
        quote_spanned! {
            input.ident.span() =>
            #trait_ident #interface_generics for Vector<N, T, A>
        }
    } else {
        quote_spanned! {
            input.ident.span() =>
        ...
        quote_spanned! {
            ...
            #item_vis #fn_sig {
                <ScalarCount<N> as #len_trait_ident<N>>
                    ::#fn_call_ident::<T, A #(, #interface_generics_args)* #(, #fn_call_generic_args)*>
                        (#(#fn_call_inputs), *)
            }
        }
    });
    ...
    let output_assoc_types = input.assoc_types.iter().map(|assoc_type| {
        ...
        quote_spanned! {
            ...
            #item_vis type #assoc_type_ident #assoc_type_generics =
                <ScalarCount<N> as #len_trait_ident<N>>
                    ::#assoc_type_ident::<T, A  #(, #interface_generics_args)* #(, #value_generic_args)*>;
        }
    });
...
fn scalar(input: &VecInterface) -> TokenStream {
    ...
    let output_fn_declarations = input
        .fns
        .iter()
        .map(|r#fn| {
            ALIGN_STRS
