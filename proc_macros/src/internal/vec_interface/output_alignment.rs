...
pub fn alignment(interface: &VecInterface) -> TokenStream {
    let modified_fn_sigs = interface.impls.iter().map(|r#impl| r#impl
        ...
        .collect::<Box<[(Signature, &VecInterfaceFn)]>>();

    let output_trait_fn_declarations = modified_fn_sigs.iter().map(|(modified_fn_sig, _)| {
        search_replace_fn(
    ...
    let output_trait_impls = LEN_STRS
        .map(|n_str| {
            ALIGN_STRS.map(|a_str| {
                let fn_impls = modified_fn_sigs.iter().map(|(modified_fn_sig, original_fn)| {
