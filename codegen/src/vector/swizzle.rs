...
pub fn write_mod(mut module: Mod) {
        ...
        for len2 in 2..=len {
            let fns = combinations(len, len2)
                ...
                .map(|indicies| {
                    ...
                    let values = indicies.iter().map(|i| format!("self.array[{i}]")).collect::<Vec<_>>().join(", ");
    ...
    for len in LENGTHS {
        let fns = partial_splits(0..len).iter().map(|split| {
            let fn_name = split
                .iter()
            ...
            if split.len() == 1 {
                ...
                if len2 == 1 {
                    formatdoc! {r#"
                        ...
                        #[inline(always)]
                        pub const fn {fn_name}_mut(&mut self) -> &mut T {{
                            &mut self[{start}]
                        }}
                    "#}
                } else {
                    formatdoc! {r#"
                        /// Returns a reference to the {comp_list} ({comp_ordinal_list}) components part of the vector.
                        #[inline(always)]
                        pub const fn {fn_name}_mut(&mut self) -> &mut Vector<{len2}, T, VecPacked> {{
                            self.get_{len2}_mut({start})
                        }}
                    "#}
                }
            } else {
                ...
                let output_fields = split.iter().map(|range| {
                    if range.len() == 1 {
                        format!("&mut (*self.as_ptr_mut().add({}))", range.start)
                    } else {
                        format!("&mut (*self.as_ptr_mut().add({}) as *mut _)", range.start)
                    }
                }).collect::<Vec<_>>().join(", ");
                ...
                formatdoc! {r#"
                    ...
                    {comp_list_lines}
                    #[inline(always)]
                    pub const fn {fn_name}_mut(&mut self) -> {output_type} {{
                        {output}
                    }}
                "#}
            }
    ...
    for len in LENGTHS {
        let fns = combinations_no_duplicates(len, len).iter().map(|indicies| {
            ...
            if indicies.len() == 1 {
                ...
                let stmts = indicies.iter().enumerate().map(|(src_idx, &dst_idx)| {
                    format!("self.array[{dst_idx}] = value.array[{src_idx}];")
                }).collect::<Vec<_>>().join("\n\t");
...
fn partial_splits(range: Range<usize>) -> Vec<Vec<Range<usize>>> {
    let start = range.start;
    let end = range.end;

    fn helper(start: usize, end: usize) -> Vec<Vec<Range<usize>>> {
        if start == end {
            return vec![]; // can stop here
        }

        let mut result = vec![]; // include the option to stop immediately

        // Try every possible next split
        for split_point in start + 1..=end {
            let first = start..split_point;
            for mut rest in helper(split_point, end) {
