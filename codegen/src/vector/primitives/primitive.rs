...
use crate::constants::{FLOAT_PRIMITIVES, INT_PRIMITIVES, LENGTHS, NUM_PRIMITIVES, PRIMARY_PRIMITIVES};
...
pub fn push_test(primitive: &str, use_stmts: &mut Vec<Tokens>, functions: &mut Vec<Tokens>) {
    ...
    functions.push(quote! {
        $(
            ...
            $(
                for &a in ["VecAligned", "VecPacked"].iter().filter(|&&a| a != "VecPacked" || PRIMARY_PRIMITIVES.contains(&primitive)) join($['\r']) =>
