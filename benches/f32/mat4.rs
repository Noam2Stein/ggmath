...
bench!(
    ...
    (
        unaligned,
        |vectors: [Vec4<f32>; TRANSFORM_BATCH], m: Mat4<f32>| vectors.map(|v| v * m)
    ),
    (
        aligned,
        |vectors: [Vec4A<f32>; TRANSFORM_BATCH], m: Mat4A<f32>| vectors.map(|v| v * m)
    ),
    (
        aligned_glam,
        |vectors: [glam::Vec4; TRANSFORM_BATCH], m: glam::Mat4| vectors.map(|v| m * v)
    ),
    (
        x4_unaligned,
        |vectors: [Vec4<f32x4>; TRANSFORM_BATCH], m: Mat4<f32x4>| vectors.map(|v| v * m)
    ),
);
