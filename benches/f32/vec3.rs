...
macro_rules! unary_operator_benchmark {
    ($group:ident: $ggmath_fn:ident, $glam_fn:ident => $op_punct:tt) => {
        #[library_benchmark]
        fn $ggmath_fn() -> ggmath::f32::FVec3 {
            black_box($op_punct black_box(ggmath::vec3!(1.0, 2.0, 3.0)))
        }

        #[library_benchmark]
        fn $glam_fn() -> glam::Vec3A {
            black_box($op_punct black_box(glam::vec3a(1.0, 2.0, 3.0)))
        }
...
macro_rules! binary_operator_benchmark {
    ($group:ident: $ggmath_fn:ident, $glam_fn:ident => $op_punct:tt) => {
        #[library_benchmark]
        fn $ggmath_fn() -> ggmath::f32::FVec3 {
            black_box(black_box(ggmath::vec3!(1.0, 2.0, 3.0)) $op_punct black_box(ggmath::vec3!(4.0, 5.0, 6.0)))
        }

        #[library_benchmark]
        fn $glam_fn() -> glam::Vec3A {
            black_box(black_box(glam::vec3a(1.0, 2.0, 3.0)) $op_punct black_box(glam::vec3a(4.0, 5.0, 6.0)))
        }
