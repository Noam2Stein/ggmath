...
bench!(
    ...
    (
        unaligned,
        |vectors: [Vec2<f32>; BATCH_LEN], q: Rot2<f32>| vectors.map(|v| v * q)
    ),
    (
        aligned,
        |vectors: [Vec2A<f32>; BATCH_LEN], q: Rot2A<f32>| vectors.map(|v| v * q)
    ),
    (
        x4_unaligned,
        |vectors: [Vec2<f32x4>; BATCH_LEN], q: Rot2<f32x4>| vectors.map(|v| v * q)
    ),
);
