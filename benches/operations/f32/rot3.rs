...
bench!(
    ...
    (
        unaligned,
        |vectors: [Vec3<f32>; BATCH_LEN], r: Rot3<f32>| vectors.map(|v| v * r)
    ),
    (
        aligned,
        |vectors: [Vec3A<f32>; BATCH_LEN], r: Rot3A<f32>| vectors.map(|v| v * r)
    ),
    (
        aligned_glam,
    ...
    (
        x4_unaligned,
        |vectors: [Vec3<f32x4>; BATCH_LEN], r: Rot3<f32x4>| vectors.map(|v| v * r)
    ),
);
