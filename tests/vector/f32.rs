...
fn test_simd_primitive_fns() {
    ...
    assert_eq!(vec2!(1.0f32).as_array(), [1.0f32; 2]);
    assert_eq!(vec3!(1.0f32).as_array(), [1.0f32; 3]);
    assert_eq!(vec4!(1.0f32).as_array(), [1.0f32; 4]);
    ...
    assert_panic!(vec2!(1.0f32, 2.0f32)[2]);
    assert_panic!(vec3!(1.0f32, 2.0f32, 3.0f32)[3]);
    assert_panic!(vec4!(1.0f32, 2.0f32, 3.0f32, 4.0f32)[4]);
    ...
    assert_panic!(&mut vec2!(1.0f32, 2.0f32)[2]);
    assert_panic!(&mut vec3!(1.0f32, 2.0f32, 3.0f32)[3]);
    assert_panic!(&mut vec4!(1.0f32, 2.0f32, 3.0f32, 4.0f32)[4]);
...
fn test_nonsimd_primitive_fns() {
    ...
    assert_eq!(vec2s!(1.0f32).as_array(), [1.0f32; 2]);
    assert_eq!(vec3s!(1.0f32).as_array(), [1.0f32; 3]);
    assert_eq!(vec4s!(1.0f32).as_array(), [1.0f32; 4]);
...
fn float_vec_eq<const N: usize, S: Simdness>(a: Vector<N, f32, S>, b: Vector<N, f32, S>) -> bool
where
...
fn test_simd_float_fns() {
    ...
    #[cfg(feature = "right")]
    {
        ...
        assert_vec!(Vec2::<f32>::RIGHT, Vec2::<f32>::X, "Vec2::RIGHT");
        assert_vec!(Vec3::<f32>::RIGHT, Vec3::<f32>::X, "Vec3::RIGHT");
        assert_vec!(Vec4::<f32>::RIGHT, Vec4::<f32>::X, "Vec4::RIGHT");

        assert_vec!(Vec2::<f32>::LEFT, Vec2::<f32>::NEG_X, "Vec2::LEFT");
        assert_vec!(Vec3::<f32>::LEFT, Vec3::<f32>::NEG_X, "Vec3::LEFT");
        assert_vec!(Vec4::<f32>::LEFT, Vec4::<f32>::NEG_X, "Vec4::LEFT");
    }
    ...
    #[cfg(feature = "left")]
    {
        ...
        assert_vec!(Vec2::<f32>::RIGHT, Vec2::<f32>::NEG_X, "Vec2::RIGHT");
        assert_vec!(Vec3::<f32>::RIGHT, Vec3::<f32>::NEG_X, "Vec3::RIGHT");
        assert_vec!(Vec4::<f32>::RIGHT, Vec4::<f32>::NEG_X, "Vec4::RIGHT");

        assert_vec!(Vec2::<f32>::LEFT, Vec2::<f32>::X, "Vec2::LEFT");
        assert_vec!(Vec3::<f32>::LEFT, Vec3::<f32>::X, "Vec3::LEFT");
        assert_vec!(Vec4::<f32>::LEFT, Vec4::<f32>::X, "Vec4::LEFT");
    }
    ...
    #[cfg(feature = "backwards")]
    {
        ...
        assert_vec!(Vec3::<f32>::FORWARDS, Vec3::<f32>::NEG_Z, "Vec3::FORWARDS");
        assert_vec!(Vec4::<f32>::FORWARDS, Vec4::<f32>::NEG_Z, "Vec4::FORWARDS");

        assert_vec!(Vec3::<f32>::BACKWARDS, Vec3::<f32>::Z, "Vec3::BACKWARDS");
        assert_vec!(Vec4::<f32>::BACKWARDS, Vec4::<f32>::Z, "Vec4::BACKWARDS");
    }

    const UNIQUE_FLOAT_VALUES: &[f32] = &[
        1.0,
        5.3,
        -1.0,
        -23.4,
        ...
        (|$vec2:ident, $vec3:ident, $vec4:ident| { $($test:tt)* }) => {
            for &x in UNIQUE_FLOAT_VALUES {
                for &y in UNIQUE_FLOAT_VALUES {
                    ...
                    (|$vec2: Vec2<f32>, $vec3: Vec3<f32>, $vec4: Vec4<f32>| { $($test)* })(vec2, vec3, vec4);
                }
            }
        };
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        assert_debug_panic!(vec2.max(vec2!(1.0, f32::NAN)));
        assert_debug_panic!(vec3.max(vec3!(1.0, f32::NAN, 1.0)));
        assert_debug_panic!(vec4.max(vec4!(1.0, f32::NAN, 1.0, 1.0)));
    });
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        assert_debug_panic!(vec2.min(vec2!(1.0, f32::NAN)));
        assert_debug_panic!(vec3.min(vec3!(1.0, f32::NAN, 1.0)));
        assert_debug_panic!(vec4.min(vec4!(1.0, f32::NAN, 1.0, 1.0)));
    });
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        ...
        assert_debug_panic!(vec2.clamp(vec2!(1.0, f32::NAN), vec2!(2.0)));
        assert_debug_panic!(vec3.clamp(vec3!(1.0, f32::NAN, 1.0), vec3!(2.0)));
        assert_debug_panic!(vec4.clamp(vec4!(1.0, f32::NAN, 1.0, 1.0), vec4!(2.0)));

        assert_debug_panic!(vec2.clamp(vec2!(1.0), vec2!(2.0, f32::NAN)));
        assert_debug_panic!(vec3.clamp(vec3!(1.0), vec3!(2.0, f32::NAN, 2.0)));
        assert_debug_panic!(vec4.clamp(vec4!(1.0), vec4!(2.0, f32::NAN, 2.0, 2.0)));
    });
}
...
fn test_nonsimd_float_fns() {
    ...
    #[cfg(feature = "right")]
    {
        ...
        assert_vec!(Vec2S::<f32>::RIGHT, Vec2S::<f32>::X, "Vec2S::RIGHT");
        assert_vec!(Vec3S::<f32>::RIGHT, Vec3S::<f32>::X, "Vec3S::RIGHT");
        assert_vec!(Vec4S::<f32>::RIGHT, Vec4S::<f32>::X, "Vec4S::RIGHT");

        assert_vec!(Vec2S::<f32>::LEFT, Vec2S::<f32>::NEG_X, "Vec2S::LEFT");
        assert_vec!(Vec3S::<f32>::LEFT, Vec3S::<f32>::NEG_X, "Vec3S::LEFT");
        assert_vec!(Vec4S::<f32>::LEFT, Vec4S::<f32>::NEG_X, "Vec4S::LEFT");
    }
    ...
    #[cfg(feature = "left")]
    {
        ...
        assert_vec!(Vec2S::<f32>::RIGHT, Vec2S::<f32>::NEG_X, "Vec2S::RIGHT");
        assert_vec!(Vec3S::<f32>::RIGHT, Vec3S::<f32>::NEG_X, "Vec3S::RIGHT");
        assert_vec!(Vec4S::<f32>::RIGHT, Vec4S::<f32>::NEG_X, "Vec4S::RIGHT");

        assert_vec!(Vec2S::<f32>::LEFT, Vec2S::<f32>::X, "Vec2S::LEFT");
        assert_vec!(Vec3S::<f32>::LEFT, Vec3S::<f32>::X, "Vec3S::LEFT");
        assert_vec!(Vec4S::<f32>::LEFT, Vec4S::<f32>::X, "Vec4S::LEFT");
    }
    ...
    #[cfg(feature = "backwards")]
    {
        ...
        assert_vec!(Vec3S::<f32>::BACKWARDS, Vec3S::<f32>::Z, "Vec3S::BACKWARDS");
        assert_vec!(Vec4S::<f32>::BACKWARDS, Vec4S::<f32>::Z, "Vec4S::BACKWARDS");
    }
        ...
        (|$vec2:ident, $vec3:ident, $vec4:ident| { $($test:tt)* }) => {
            for &x in UNIQUE_FLOAT_VALUES {
                for &y in UNIQUE_FLOAT_VALUES {
                    ...
                    (|$vec2: Vec2S<f32>, $vec3: Vec3S<f32>, $vec4: Vec4S<f32>| { $($test)* })(vec2, vec3, vec4);
                }
            }
        };
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        assert_debug_panic!(vec2.max(vec2s!(1.0, f32::NAN)));
        assert_debug_panic!(vec3.max(vec3s!(1.0, f32::NAN, 1.0)));
        assert_debug_panic!(vec4.max(vec4s!(1.0, f32::NAN, 1.0, 1.0)));
    });
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        assert_debug_panic!(vec2.min(vec2s!(1.0, f32::NAN)));
        assert_debug_panic!(vec3.min(vec3s!(1.0, f32::NAN, 1.0)));
        assert_debug_panic!(vec4.min(vec4s!(1.0, f32::NAN, 1.0, 1.0)));
    });
    ...
    test_for_combinations!(|vec2, vec3, vec4| {
        ...
        assert_debug_panic!(vec2.clamp(vec2s!(1.0, f32::NAN), vec2s!(2.0)));
        assert_debug_panic!(vec3.clamp(vec3s!(1.0, f32::NAN, 1.0), vec3s!(2.0)));
        assert_debug_panic!(vec4.clamp(vec4s!(1.0, f32::NAN, 1.0, 1.0), vec4s!(2.0)));

        assert_debug_panic!(vec2.clamp(vec2s!(1.0), vec2s!(2.0, f32::NAN)));
        assert_debug_panic!(vec3.clamp(vec3s!(1.0), vec3s!(2.0, f32::NAN, 2.0)));
        assert_debug_panic!(vec4.clamp(vec4s!(1.0), vec4s!(2.0, f32::NAN, 2.0, 2.0)));
    });
}
