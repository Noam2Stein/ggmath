...
pub fn primitive_tests(a: T, b: T, c: T, d: T) {
    ...
    assert!(size_of::<Vec2t>() == size_of::<[T; 2]>());
    assert!(size_of::<Vec3t>() == size_of::<[T; 3]>() || size_of::<Vec3t>() == size_of::<[T; 4]>());
    assert!(size_of::<Vec4t>() == size_of::<[T; 4]>());

    assert!(align_of::<Vec2t>() == align_of::<T>() || align_of::<Vec2t>() == size_of::<Vec2t>());
    assert!(align_of::<Vec3t>() == align_of::<T>() || align_of::<Vec3t>() == size_of::<Vec3t>());
    assert!(align_of::<Vec4t>() == align_of::<T>() || align_of::<Vec4t>() == size_of::<Vec4t>());

    assert_eq!(Vec2t::from_array([a, b]).to_array(), [a, b]);
    assert_eq!(Vec3t::from_array([a, b, c]).to_array(), [a, b, c]);
    assert_eq!(Vec4t::from_array([a, b, c, d]).to_array(), [a, b, c, d]);

    assert_eq!(Vec2t::splat(a).to_array(), [a; 2]);
    assert_eq!(Vec3t::splat(a).to_array(), [a; 3]);
    assert_eq!(Vec4t::splat(a).to_array(), [a; 4]);

    assert_eq!(Vec2t::from_fn(|i| [a, b][i]).to_array(), [a, b]);
    assert_eq!(Vec3t::from_fn(|i| [a, b, c][i]).to_array(), [a, b, c]);
    assert_eq!(Vec4t::from_fn(|i| [a, b, c, d][i]).to_array(), [a, b, c, d]);
    ...
    assert_eq!(vec4!(a, b, c, d).as_mut_array(), &mut [a, b, c, d]);
    ...
    assert_eq!(Vec2t::from([a, b]), vec2!(a, b));
    assert_eq!(Vec3t::from([a, b, c]), vec3!(a, b, c));
    assert_eq!(Vec4t::from([a, b, c, d]), vec4!(a, b, c, d));
    ...
    assert_eq!(
        (&vec2!(a, b)).into_iter().copied().collect::<Vec<T>>(),
        vec![a, b]
    );
    assert_eq!(
        (&vec3!(a, b, c)).into_iter().copied().collect::<Vec<T>>(),
        vec![a, b, c]
    );
    assert_eq!(
        (&vec4!(a, b, c, d))
    ...
    assert_eq!(Vec2t::default(), vec2!(T::default()));
    assert_eq!(Vec3t::default(), vec3!(T::default()));
    assert_eq!(Vec4t::default(), vec4!(T::default()));
