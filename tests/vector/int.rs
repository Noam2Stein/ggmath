...
macro_rules! test_int_api {
    ...
    (@body $dollar:tt dollar $T:ty, $S:ident) => {{
        ...
        assert_eq_debug_panic!(-vec2t!(1, <$T>::MIN), vec2t!(-1, -<$T>::MIN), "vec2 neg overflow");
        assert_eq_debug_panic!(-vec3t!(1, <$T>::MIN, 1), vec3t!(-1, -<$T>::MIN, -1), "vec3 neg overflow");
        assert_eq_debug_panic!(-vec4t!(1, <$T>::MIN, 1, 1), vec4t!(-1, -<$T>::MIN, -1, -1), "vec4 neg overflow");
        ...
        assert_eq_debug_panic!(vec2t!(1, <$T>::MIN) + vec2t!(3, -1), vec2t!(4, <$T>::MAX), "vec2 overflowing add");
        assert_eq_debug_panic!(vec3t!(1, <$T>::MIN, 1) + vec3t!(3, -1, 1), vec3t!(4, <$T>::MAX, 2), "vec3 overflowing add");
        assert_eq_debug_panic!(vec4t!(1, <$T>::MIN, 1, 1) + vec4t!(3, -1, 1, 1), vec4t!(4, <$T>::MAX, 2, 2), "vec4 overflowing add");

        assert_eq_debug_panic!(vec2t!(1, <$T>::MIN) + vec2t!(3, -3), vec2t!(4, <$T>::MAX - 2), "vec2 overflowing add");
        assert_eq_debug_panic!(vec3t!(1, <$T>::MIN, 1) + vec3t!(3, -3, 1), vec3t!(4, <$T>::MAX - 2, 2), "vec3 overflowing add");
        assert_eq_debug_panic!(vec4t!(1, <$T>::MIN, 1, 1) + vec4t!(3, -3, 1, 1), vec4t!(4, <$T>::MAX - 2, 2, 2), "vec4 overflowing add");
        ...
        assert_eq_debug_panic!(vec2t!(21, <$T>::MAX) - vec2t!(3, -1), vec2t!(18, <$T>::MIN), "overflowing sub");
        assert_eq_debug_panic!(vec3t!(21, <$T>::MAX, 1) - vec3t!(3, -1, 1), vec3t!(18, <$T>::MIN, 0), "overflowing sub");
        assert_eq_debug_panic!(vec4t!(21, <$T>::MAX, 1, 1) - vec4t!(3, -1, 1, 1), vec4t!(18, <$T>::MIN, 0, 0), "overflowing sub");

        assert_eq_debug_panic!(vec2t!(21, <$T>::MAX) - vec2t!(3, -3), vec2t!(18, <$T>::MIN + 2), "overflowing sub");
        assert_eq_debug_panic!(vec3t!(21, <$T>::MAX, 1) - vec3t!(3, -3, 1), vec3t!(18, <$T>::MIN + 2, 0), "overflowing sub");
        assert_eq_debug_panic!(vec4t!(21, <$T>::MAX, 1, 1) - vec4t!(3, -3, 1, 1), vec4t!(18, <$T>::MIN + 2, 0, 0), "overflowing sub");
        ...
        assert_eq_debug_panic!(vec2t!(21, <$T>::MIN) * vec2t!(-1), vec2t!(-21, -<$T>::MIN), "overflowing mul");
        assert_eq_debug_panic!(vec3t!(21, <$T>::MIN, 1) * vec3t!(-1), vec3t!(-21, -<$T>::MIN, -1), "overflowing mul");
        assert_eq_debug_panic!(vec4t!(21, <$T>::MIN, 1, 1) * vec4t!(-1), vec4t!(-21, -<$T>::MIN, -1, -1), "overflowing mul");
        ...
        assert_eq_debug_panic!(vec2t!(1, <$T>::MIN) / vec2t!(1, -1), vec2t!(1, -<$T>::MIN), "vec2 overflowing div");
        assert_eq_debug_panic!(vec3t!(1, <$T>::MIN, 1) / vec3t!(1, -1, 1), vec3t!(1, -<$T>::MIN, 1), "vec3 overflowing div");
        assert_eq_debug_panic!(vec4t!(1, <$T>::MIN, 1, 1) / vec4t!(1, -1, 1, 1), vec4t!(1, -<$T>::MIN, 1, 1), "vec4 overflowing div");
